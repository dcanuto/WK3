...
function main()
    ...
    # RTPS allocators
    p = 0.8; # relaxation amount, ∃ [0.5, 0.95]
        ...
        for nn in 1:length(tf)+1
            ...
            if nn <= length(tf)
                ...
                for i = 1:length(systems)
                    ...
                    if θ[i][5] >= θ[i][6]
                        warn("τ1 > τ2 for member $i. Setting τ1 to previous value.")
                        θ[i][5] = θprev[i][5];
                    end
                    θ[i] = θ[i]./θs;
                end
                ...
                for i = 1:length(soln)
                    ...
                    if mparams[i].Zc <= error.lb[1]
                        println("Warning: analysis Zc non-positive for member $i.
                            Setting to lower bound of $(error.lb[1]) s.")
                        mparams[i].Zc = error.lb[1];
                    end
                    if mparams[i].R <= error.lb[2]
                        println("Warning: analysis R non-positive for member $i.
                            Setting to lower bound of $(error.lb[2]) s.")
                        mparams[i].R = error.lb[2];
                    end
                    if mparams[i].C <= error.lb[3]
                        println("Warning: analysis C non-positive for member $i.
                            Setting to lower bound of $(error.lb[3]) s.")
                        mparams[i].C = error.lb[3];
                    end
                    if mparams[i].V0 <= error.lb[4]
                        println("Warning: analysis V0 non-positive for member $i.
                            Setting to lower bound of $(error.lb[4]) s.")
                        mparams[i].V0 = error.lb[4];
                    end
                    if mparams[i].t1 <= error.lb[5]
                        println("Warning: analysis τ1 non-positive for member $i.
                            Setting to lower bound of $(error.lb[5]) s.")
                        mparams[i].t1 = error.lb[5];
                    end
                    if mparams[i].t2 <= error.lb[6]
                        println("Warning: analysis τ2 non-positive for member $i.
                            Setting to lower bound of $(error.lb[6]) s.")
                        mparams[i].t2 = error.lb[6];
                    end
                    if mparams[i].m1 <= error.lb[7]
                        println("Warning: analysis m1 non-positive for member $i.
                            Setting to lower bound of $(error.lb[7]) s.")
                        mparams[i].m1 = error.lb[7];
                    end
                    if mparams[i].m2 <= error.lb[8]
                        println("Warning: analysis m2 non-positive for member $i.
                            Setting to lower bound of $(error.lb[8]) s.")
                        mparams[i].m2 = error.lb[8];
                    end
                    if mparams[i].Emax <= error.lb[9]
                        println("Warning: analysis Emax non-positive for member $i.
                            Setting to lower bound of $(error.lb[9]) s.")
    ...
    # output
    return systems,tout,xout,yout,innov,θout
end
